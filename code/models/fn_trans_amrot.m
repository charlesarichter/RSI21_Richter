function output = fn_trans_amrot(coeffs,eqdata)
%% Unpack Data
force = eqdata(:,1);
...
%% Trans_AMRot: Complexity ZJW - 17
Ct = coeffs(1); % 1.2
% Cr = coeffs(2); % pi;
Camrot = coeffs(2);
m22 = Camrot*pi*rho*(chord/2).^2;
...
Fxprime = m22.*defdot.*R2.*-wy - rho*-wy.*(-2*Ct.*a.*R3.*wz.*-wy./sqrt(wz.^2+wy.^2));
Fyprime = rho*wz.*(-2*Ct.*a.*R3.*wz.*-wy./sqrt(wz.^2+wy.^2));
