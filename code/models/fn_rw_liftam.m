function output = fn_rw_liftam(coeffs,eqdata)
%% Unpack Data
force = eqdata(:,1);
...
if length(coeffs)==3
    ...
    betaz = coeffs(2);
    betazw = coeffs(3);
    lambdaz = betaz*pi*rho*(chord/2).^2; % kg/m^3 * m^2
    lambdazw = betazw*-pi*rho*(chord/2).^2.*(chord/2-upperchord); % kg/m^3 * m^2 * m
else
    Clmax = coeffs(1);
...
Z02 = -lambdazw.*wxdot.*(outerspan-innerspan);
% Z0spanwise = Z01+Z02;
% AMrjw = Z0spanwise.*-sin(def);
% output = Lrjw+AMrjw+inertial+inerttheta;
AMTrans = Z01.*-sin(def);
AMRotdot = Z02.*-sin(def);
output = Lrjw+AMTrans+AMRotdot+inertial+inerttheta;