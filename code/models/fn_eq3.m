function output = fn_eq3(coeffs,eqdata)
%% Unpack Data
force = eqdata(:,1);
...
% chord = 1e-3.*eqdata(:,22);lowerchord = 1e-3.*eqdata(:,20);upperchord =
% 1e-3.*eqdata(:,21);
...
C3 = coeffs(3);
% C4 = coeffs(4);
output = C1.*span.*Ycm.*defdot.*defdot.*cos(def - def.*def)...
    + C2.*strokedot.*span.*Xcm.*def...
    - C3.*span.*devddot;

% Coeffs = [1.4180717e-8 1.2409225e-7 1.0660193e-6]
% 1.4180717e-8.*span.*Ycm.*defdot.*defdot.*cos(def - def.*def) +
% 1.2409225e-7.*strokedot.*span.*Xcm.*def - 1.0660193e-6.*span.*devddot