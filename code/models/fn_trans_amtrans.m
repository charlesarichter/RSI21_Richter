function output = fn_trans_amtrans(coeffs,eqdata)
%% Unpack Data
force = eqdata(:,1);
...
%% Trans_AMRot: Complexity ZJW - 15
Ct = coeffs(1); % 1.2
% Cr = coeffs(2); % pi;
Camtrans = coeffs(2);
m22 = Camtrans*pi*rho*(chord/2).^2;
...
Fxprime = -rho*-wy.*(-2*Ct.*a.*R3.*wz.*-wy./sqrt(wz.^2+wy.^2));
Fyprime = -m22.*R2.*(-wydot+wx.*wz) + rho*wz.*(-2*Ct.*a.*R3.*wz.*-wy./sqrt(wz.^2+wy.^2));
